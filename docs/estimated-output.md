...
The "Estimated output" tool ([pkg/widgets/estimatedoutput/](../pkg/widgets/estimatedoutput/)) has
two independent halves that share one graph:
...
estimators ([estimatedoutput.go:69](../pkg/widgets/estimatedoutput/estimatedoutput.go#L69)); anything
else shows "No output estimator for ECU …".
...
**T7 / T8** ([t7.go](../pkg/widgets/estimatedoutput/t7.go),
[t8.go](../pkg/widgets/estimatedoutput/t8.go)) follow the ECU's own request chain:
...
high-RPM roll-off (1.00 below 5060 rpm, 0.99, 0.95, 0.94, 0.85 at ≥6000 rpm —
[t7.go:383](../pkg/widgets/estimatedoutput/t7.go#L383)), or the binary's nominal torque map if you
tick that option. Power is `torque × rpm / 7024` in whole hp, using integer division exactly like
the original suites, so the curve steps in 1 hp increments. Note 7024 is the *metric* hp (PS)
constant (60·735.5 W/2π); the curves are labeled plain "hp" accordingly — multiply by 0.986 if you
want true brake hp.

**T5** ([t5.go](../pkg/widgets/estimatedoutput/t5.go)) has no torque model in the binary, so it works
off boost instead: it reads the WOT column of the boost request map (`Tryck_mat!` or `Tryck_mat_a!`),
converts it to gauge bar, and looks torque up in a fixed torque-vs-boost table per turbo family.
The Table tab is the boost request in bar over throttle × RPM. Injector DC replays the T5 firmware's
own injection-time math at a fixed ~20 °C intake temperature and 13.0 V battery.
...
See [fromlog.go](../pkg/widgets/estimatedoutput/fromlog.go).
