...
func (s *VBar) CreateRenderer() fyne.WidgetRenderer {
	...
	// Build tick lines using gradient across full range (Min..Max)
...
func (r *VBarRenderer) Layout(space fyne.Size) {
	...
	r.displayText.Move(fyne.Position{
		X: r.layoutValues.middle + r.layoutValues.displayTextX,
	...
	norm := r.clampNorm(r.value)               // 0..1
	barHeight := norm * float32(r.size.Height) // px
	r.bar.Resize(fyne.Size{Width: r.size.Width, Height: barHeight})
	r.bar.Move(fyne.Position{X: 0, Y: r.size.Height - barHeight})
}
