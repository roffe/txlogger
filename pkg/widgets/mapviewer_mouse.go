...
func (mv *MapViewer) MouseDown(event *desktop.MouseEvent) {
	...
	// Handle left mouse button without modifiers
	if event.Button == desktop.MouseButtonPrimary && event.Modifier == 0 {
		mv.handlePrimaryClick(event)
		return
	}

	// Handle left mouse button with shift modifier
	if event.Button == desktop.MouseButtonPrimary && event.Modifier == fyne.KeyModifierShift {
		mv.handlePrimaryClickWithShift(event)
		return
	}

	if event.Button == desktop.MouseButtonPrimary && event.Modifier == fyne.KeyModifierControl {
		log.Println("Control")
		return
	}

	if event.Button == desktop.MouseButtonSecondary && event.Modifier == 0 {
