# 2.2.0
- Custom colors: put a theme.json next to txlogger.exe (or next to the AppImage on Linux) to override the theme colors and sizes. Only list what you want to change, e.g. {"Colors": {"background": "#101010ff", "primary": "#e53935ff"}}; everything else keeps its normal value. Colors are #rrggbbaa. The full list of names with their current values is in https://github.com/roffe/txlogger/blob/master/pkg/theme/theme.default.json. Gauges, the map viewer and the plotter draw their own colors and are not affected yet
- Flashing through a Kvaser or a J2534 interface is much faster. A Trionic 7 fast flash went from 60 seconds to 18 on a Kvaser (the legacy mode took 90), and from 32 to 18 on a MongoosePro. txlogger used to wait for the interface to confirm each frame was on the bus before sending the next, a USB round trip for every one of the ~80,000 frames a flash takes. Frames are now handed to the driver and sent straight on, which is how TrionicCANLib has always done it. When the interface's transmit queue is full, txlogger waits for it to drain instead of failing the block
- Kvaser interfaces on Linux are loaded without cgo now, like J2534. txlogger finds Kvaser's libcanlib when it starts, so the Kvaser SDK doesn't need to be installed to build it
- Fixed the DTC reader on Trionic 7 sometimes failing with "ReadDTCByStatus[3]: context deadline exceeded" while J1979 read the same codes fine. If one frame of a multi-frame DTC reply was lost or late, the T7 silently abandons the rest of the reply, and txlogger gave up. It now asks again, up to three times, and a fresh request makes the ECU start its reply over
- Trionic 7 closed binaries: opening a map while connected no longer tries to read it from the ECU. On a closed binary the calibration lives in flash, which the ECU refuses to hand out by address, so every map open ended in an error — and on MapTun-tuned ECUs the read request crashes the ECU and it resets, killing the logging session. Those maps now only load from and save to the file, and the Load ECU / Save ECU buttons are hidden for them. Open binaries, with the calibration in RAM, work as before
- Fixed a window drifting away from the mouse when dragged against the edge of the workspace. Once the window stopped at the edge, the extra mouse travel was lost, so on the way back the window no longer sat under the spot you grabbed. It now waits at the edge until the mouse returns to that spot, the same way resizing already worked. The maximize button also turns into a restore button while a window is maximized, buttons have their rounded corners back, and window shadows are drawn by the GPU instead of eight gradient images per window, with a bigger shadow on the active window
- J2534 interfaces work on Linux again. Drivers are picked up from ~/.passthru/*.json (the same layout other Linux J2534 tools use, with FUNCTION_LIB pointing at the .so) and show up as "J2534 #n <vendor> <name>" in the adapter list, exactly like the registry-listed DLLs on Windows. The library is loaded without cgo now
...
- Fixed Trionic 7 flashing failing on the very last bytes of some binaries, leaving the top of the PI area unwritten. When the last block of a region was only a few bytes long it reached the ECU while it was still programming the block before, and the ECU answered "busy, repeat request". txlogger then restarted the download at that address instead of repeating the block, but a busy T7 reads the new download address out of the block it is holding, so it got an address made of flash data and aborted programming. The block is now resent as the ECU expects
- CAN flasher: a Trionic 7 left in programming mode by a failed flash can now be flashed again without pulling the fuse. If the earlier attempt died before the erase finished, the ECU refused every later erase until it was power cycled; and retrying within a few seconds of a failure used to time out on "Data initialization failed" because the ECU still held the old session. txlogger now ends the stale session and restarts the programming sequence
