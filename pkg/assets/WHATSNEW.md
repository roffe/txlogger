...
- Added "Import address table from another binary" under the Tools menu (T7): some tuners re-point entries in the symbol address table so tools show the wrong data for the tuning maps, while the code and the real maps are untouched. Load the tuned file, pick any stock binary of the same software (same trace, the part of the version before the dot, e.g. EI03F01C) as donor, and the clean table is copied over, the checksum recomputed and the result saved as a new file. The original is never overwritten
